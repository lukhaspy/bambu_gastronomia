            ...
            <div class="card-header">
                <div class="row align-items-center">
                    ...
                    <div class="col-4 text-right">
                        <a href="{{ route('sales.show', [$sale->id]) }}" class="btn btn-sm btn-primary">Volver</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <div class="pl-lg-4">
                        ...
                        <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                            ...
                            <select name="product_id" id="input-product" class="form-select form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" required>
                                ...
                                <option value="{{$product['id']}}" selected>[{{ $product->category->name }}] | [{{getUnity($product->unity)}}] | [Stock:{{($product->stock)}}] {{ $product->name }} - Precio: {{ format_money($product->price) }}
                                    @else
                                <option value="{{$product['id']}}">[{{ $product->category->name }}] | [{{getUnity($product->unity)}}] | [Stock:{{($product->stock)}}] {{ $product->name }} - Precio: {{ format_money($product->price) }} @endif
                                    @endforeach
                            </select>
...
<script>
    ...
    function updateTotal() {
        input_total.value = (parseInt(input_qty.value) * parseFloat(input_price.value)) + " Gs";
    }
</script>
@endpush