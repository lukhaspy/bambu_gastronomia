@extends('layouts.app', ['page' => 'Editar Ingreso', 'pageSlug' => 'incomes', 'section' => 'transactions'])
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Editar Ingreso</h3>
                        </div>
                        <div class="col-4 text-right">
                            <a href="{{ route('transactions.type', ['type' => 'income']) }}" class="btn btn-sm btn-primary">Volver</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('transactions.update', $transaction) }}" autocomplete="off">
                        ...
                        <h6 class="heading-small text-muted mb-4">Info del Ingreso</h6>
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-title">Título</label>
                                <input type="text" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{ old('title', $transaction->title) }}" required autofocus>
                            ...
                            <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                ...
                                <input type="number" step=".01" name="amount" id="input-amount" class="form-control form-control-alternative" value="{{ old('amount', $transaction->amount) }}" min="0" required>
