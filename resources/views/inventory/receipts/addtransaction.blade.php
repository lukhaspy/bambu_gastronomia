@extends('layouts.app', ['page' => 'Nuevo Movimiento', 'pageSlug' => 'receipts', 'section' => 'transactions'])
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Nuevo Movimiento</h3>
                        </div>
                        <div class="col-4 text-right">
                            <a href="{{ route('receipts.show', $receipt) }}" class="btn btn-sm btn-primary">Volver a la compra</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('inventory.receipts.transaction.store', $receipt) }}" autocomplete="off">
                        @csrf
                        <input type="hidden" name="receipt_id" value="{{ $receipt->id }}">
                        <input type="hidden" name="provider_id" value="{{ $receipt->provider_id }}">
                        <input type="hidden" name="user_id" value="{{ Auth::id() }}">

                        <h6 class="heading-small text-muted mb-4">Info del Movimiento</h6>
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-method">Tipo de Movimiento</label>
                                <select name="type" id="input-method" class="form-control form-control-alternative{{ $errors->has('type') ? ' is-invalid' : '' }}" required>
                                    @foreach (['payment' => 'Pagos'] as $type => $title)
                                    @if($type == old('type'))
                            ...
                            <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                ...
                                <input type="number" step=".01" name="amount" id="input-amount" class="form-control form-control-alternative" placeholder="Amount" value="{{ old('amount', $receipt->products->sum('total_amount')) }}" required>
