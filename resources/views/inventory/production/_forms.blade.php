...
<div class="pl-lg-4">
    ...
    <div class="form-group{{ $errors->has('product_category_id') ? ' has-danger' : '' }}">
        ...
        {!! Form::select('product_category_id', $categories, null, ['class' => 'form-select form-control-alternative', 'required', 'placeholder' => 'Seleccione una categoría']) !!}
    ...
    <div class="row">
        <div class="col-4">
            <div class="form-group{{ $errors->has('unity') ? ' has-danger' : '' }}">
                ...
                {!! Form::select('unity', getUnities(), null, ['class' => 'form-select form-control-alternative', 'required']) !!}
...
@push('js')
    <script>
        ...
        @isset($production)
            var count = {{ $production->materials->count() + 1}}
            fillTable()
        @else
            var count = 4
        @endif
